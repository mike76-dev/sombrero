...
func TestIndexdClient_FileLifecycle(t *testing.T) {
	...
	c := newIndexdClient(db, newFakeBackend(), share.Name, 1, 0, PackingOptions{})
...
func TestIndexdClient_RenameDuringMixedUpload(t *testing.T) {
	...
	c := newIndexdClient(db, backend, share.Name, 1, 0, PackingOptions{})
	t.Cleanup(func() { _ = c.Close() })
...
func TestIndexdClient_PackedSlab(t *testing.T) {
	...
	fb := newFakeBackend()
	c := newIndexdClient(db, fb, share.Name, 1, 0, PackingOptions{})
	t.Cleanup(func() { _ = c.Close() })
