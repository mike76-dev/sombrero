...
// stampSessionID names the session in every SMB2 header of the message that names none, which a
// lease break does not ([MS-SMB2] 3.2.5.1.1: a client drops the connection over a header naming
// another session). It walks the message as SMB2 headers, so it must run before compression.
